    ...
    public class ToolDefault : Tool
    {
        ...
        void DragUpdate()
        {
            ...
            if (_mode == Mode.Position)
            {
                ...
                {
                    case DragState.Both:
                        dragAmount = dragAmount.AddPosition(mousePos - _mousePosPrev);
                        break;

                    case DragState.Horizontal:
                        dragAmount = dragAmount.AddPosition(new Vector2(mousePos.X - _mousePosPrev.X, 0));
                        break;

                    case DragState.Vertical:
                        dragAmount = dragAmount.AddPosition(new Vector2(0, mousePos.Y - _mousePosPrev.Y));
                        break;
                }
                _mousePosPrev = mousePos;
            }
            else if (_mode == Mode.Rotate)
            {
                ...
                angle = MathEx.VectorToAngleReversed(mousePos - _translator.WorldTransform.Position);
                anglePrev = MathEx.VectorToAngleReversed(_mousePosPrev - _translator.WorldTransform.Position);
        ...
        {
            base.Enable();
            _translator = new Doodad("Translator");
            Controller.Level.Doodads.Add(_translator);
