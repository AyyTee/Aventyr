    ...
    public class Program
    {
        ...
        private static void CreateAtlas(int fontCount)
        {
            ...
            var modelFiles = Directory
                .GetFiles(Paths.Models, "*.obj", SearchOption.AllDirectories)
                .Select(item => new ModelFile(item))
                .ToImmutableArray();
        ...
        static void GenerateAssetCode(string @namespace, string @class, string codeBody)
        {
            File.WriteAllText(Path.Combine(Paths.GameSource, @namespace, $"{@class}.cs"),
