    ...
    public static class Ray
    {
        ...
        static Result _rayCast(Transform2 worldTransform, Transform2 velocityCurrent, IEnumerable<IPortalRenderable> portals, double movementLeft, IPortalRenderable portalPrevious, Settings settings, int count)
        {
            ...
            if (portalNearest != null)
            {
                ...
                begin = begin.WithPosition((Vector2)intersectNearest.Position);
        ...
        {
            begin = begin.WithPosition(begin.Position + velocity.Position.Normalized() * (float)movementLeft);
        ...
        static Transform2 AddMargin(IEnumerable<IPortalRenderable> portals, IPortalRenderable portalPrevious, Transform2 transform, Transform2 velocity)
        {
            foreach (var p in portals)
            {
                ...
                if (distanceToPortal < Portal.EnterMinDistance)
                {
                    ...
                    transform = transform.WithPosition(transform.Position + pos);
                    break;
                }
            }
            return transform;
