        ...
        public Controller(IVirtualWindow window)
        {
            ...
            _menu.Root.Children = new IElement[]
            {
                new StackFrame(width: _ => 200, spacing: _ => 5)
                {
                    ...
                    {
                        new Text(ElementEx.AlignX(0.5f), ElementEx.AlignY(0.5f), _ => _window.Fonts.Inconsolata, _ => "Start Game")
                    },
                    new Button(height: _ => 80, onClick: StartLevelEditor)
                    {
                        new Text(ElementEx.AlignX(0.5f), ElementEx.AlignY(0.5f), _ => _window.Fonts.Inconsolata, _ => "Level Editor")
                    },
                    new Button(height: _ => 80, onClick: _window.Exit)
                    {
                        new Text(ElementEx.AlignX(0.5f), ElementEx.AlignY(0.5f), _ => _window.Fonts.Inconsolata, _ => "Exit")
                    }
                }
            }.ToImmutableList();
